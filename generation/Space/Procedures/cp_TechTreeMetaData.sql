...
-- =============================================================================
ALTER PROCEDURE [dbo].[cp_TechTreeMetaData]
	 @Operation     	[INT]                	= 0
	,@Active        	[BIT]                	= NULL
...
AS
 ...
 BEGIN TRY 
	...
	IF( @Operation = 1 )
		BEGIN
			SELECT	 [Active]
					...
					,[SysId]
					,[TechTreeSysId]
			FROM	[Space].[dbo].[TechTreeMetaData]
			WHERE	[SysId] = @SysId
		END
	...
	IF( @Operation = 4 )
		BEGIN
			UPDATE	[Space].[dbo].[TechTreeMetaData]
			SET		 [Active]        	= @Active
					,[KeyPath]       	= @KeyPath
					,[KeyValue]      	= @KeyValue
					,[PublicId]      	= @PublicId
					,[TechTreeSysId] 	= @TechTreeSysId
			WHERE	[SysId] = @SysId
		END
