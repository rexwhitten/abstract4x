...
-- =============================================================================
ALTER PROCEDURE [dbo].[cp_AsteroidFiledMetaData]
	 @Operation          	[INT]                	= 0
	,@Active             	[BIT]                	= NULL
...
AS
 ...
 BEGIN TRY 
	...
	IF( @Operation = 1 )
		BEGIN
			SELECT	 [Active]
					...
					,[PublicId]
					,[SysId]
			FROM	[Space].[dbo].[AsteroidFiledMetaData]
			WHERE	[SysId] = @SysId
		END
	...
	IF( @Operation = 4 )
		BEGIN
			UPDATE	[Space].[dbo].[AsteroidFiledMetaData]
			SET		 [Active]             	= @Active
					,[AsteroidFiledSysId] 	= @AsteroidFiledSysId
					,[KeyPath]            	= @KeyPath
					,[KeyValue]           	= @KeyValue
					,[PublicId]           	= @PublicId
			WHERE	[SysId] = @SysId
	...
	IF( @Operation = 5 )
		BEGIN
			INSERT INTO
					[Space].[dbo].[AsteroidFiledMetaData] (
						 [Active]
						,[AsteroidFiledSysId]
						,[KeyPath]
						,[KeyValue]
